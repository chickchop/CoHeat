...
class CoHeat(nn.Module):
    ...
    def get_aff_graph_ori(self):
        ...
        # edge_index for GraphSAGE
        coo = item_level_graph.tocoo()
        self.aff_edge_index = (
            torch.LongTensor(coo.row).to(self.device),
            torch.LongTensor(coo.col).to(self.device)
        )
    ...
    def get_hist_graph_ori(self):
        ...
        coo2 = bundle_level_graph.tocoo()
        self.hist_edge_index = (
            torch.LongTensor(coo2.row).to(self.device),
            torch.LongTensor(coo2.col).to(self.device)
        )
    ...
    def one_propagate_sample(self, edge_index, A_feat, B_feat, num_samples=10):
        """
        Fast GraphSAGE-style neighbor sampling + aggregation
        - edge_index: (src, dst) 두 LongTensor, shape = (E,)
        - A_feat: (num_A, D), B_feat: (num_B, D)
        """
        # 1) concat
        features = torch.cat((A_feat, B_feat), dim=0)  # (N, D)
        N, D = features.size()
        src, dst = edge_index  # both on same device as features

        # 2) assign random scores to edges and permute
        scores = torch.rand(src.size(0), device=features.device)
        perm = torch.argsort(scores)
        src_p, dst_p = src[perm], dst[perm]

        # 3) compute how many edges per dst node
        counts = torch.bincount(dst_p, minlength=N)      # (N,)
        # compute offsets: for each node u, its edges start at offsets[u]
        offsets = torch.cat([torch.zeros(1, dtype=torch.long, device=features.device),
                             counts.cumsum(0)[:-1]], dim=0)   # (N,)

        # 4) for each edge idx i, its position within its dst-group is:
        idx_range = torch.arange(src_p.size(0), device=features.device)
        rel_pos  = idx_range - offsets[dst_p]
        # keep only first num_samples edges per dst
        mask = rel_pos < num_samples
        src_s, dst_s = src_p[mask], dst_p[mask]

        # 5) aggregate neighbor features via scatter_mean
        neigh_feats = features[src_s]  # (M, D)
        out = scatter_mean(neigh_feats, dst_s, dim=0, dim_size=N)  # (N, D)

        # 6) self-loop + normalize (divide by layers+1 if stacking)
        out = out + features
        out = F.normalize(out / float(self.num_layers + 1), p=2, dim=1)

        # 7) split back
        A_out = out[:A_feat.size(0)]
        B_out = out[A_feat.size(0):]
        return A_out, B_out
    ...
    def propagate_sample(self, test=False, num_samples=10):
        # Affiliation‐view
        aff_src, aff_dst = self.aff_edge_index  # e.g. defined in __init__
        aff_u, aff_i = self.one_propagate_sample(
            (aff_src, aff_dst),
            self.users_feature,
            self.items_feature,
            num_samples
        ...
        aff_b = torch.matmul(
            self.bundle_agg_graph_ori if test else self.bundle_agg_graph,
            aff_i
        )

        # History‐view
        hist_src, hist_dst = self.hist_edge_index
        hist_u, hist_b = self.one_propagate_sample(
            (hist_src, hist_dst),
            self.users_feature,
        ...
        return [aff_u, hist_u], [aff_b, hist_b]
